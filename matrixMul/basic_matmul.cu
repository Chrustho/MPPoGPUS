...
__global__ void MatrixMulBasic(float *d_P, float *d_M, float *d_N, int j, int k, int l)
{
    ...
    if (Row < j && Col < l)
    {
        ...
        for (int t = 0; t < k; ++t)
        {
            Pvalue += d_M[(size_t)Row * k + t] * d_N[(size_t)t * l + Col];
        }
...
{
    printf("basic_matmul: A=(%d x %d), B=(%d x %d), C=(%d x %d)\n",
    ...
    FILE *fout = fopen("basic_timings.csv", "w");
    if (!fout)
    {
        fprintf(stderr, "Cannot open basic_timings.csv for writing\n");
        return 1;
    }
    ...
    for (i = 0; i < 3; ++i)
    {
        ...
        MatrixMulBasic<<<grid, block>>>(d_C, d_A, d_B, ROWS_A, COLS_A, COLS_B);
        CUDA_CHECK(cudaGetLastError());
        ...
        printf("Basic block %dx%d: %.3f ms, %.3f GFLOPS, correct=%s\n", blk, blk, ms, gflops, ok ? "YES" : "NO");
        fprintf(fout, "Basic,%d,%.6f,%.6f,%d\n", blk, ms, gflops, ok);
    ...
    FILE *fr = fopen("basic_roofline.dat", "w");
    if (fr)
    {
    ...
    printf("basic_matmul done. Files: basic_timings.csv, basic_roofline.dat\n");
